...
export function glottalPulse(phase, effort = 0.5) {
  ...
  const Tp = 0.5 - e * 0.2
  const Tn = 0.25 - e * 0.17
  const NORM = 0.1
  if (phase < Tp) return NORM * 0.5 * (Math.PI / Tp) * Math.sin(Math.PI * phase / Tp)
  if (phase < Tp + Tn) return -NORM * (Math.PI / (2 * Tn)) * Math.sin(Math.PI * (phase - Tp) / (2 * Tn))
  return 0
}
